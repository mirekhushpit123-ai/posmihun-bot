...
async def cmd_start(message: types.Message):
    ...
    welcome_text = (
        "Привіт! Я бот ПоСмІхУнЧиК. "
        "Я допомагаю створювати чудовий настрій у чаті, надсилаючи ранкові привітання, "
        "мудрі слова та побажання доброго вечора за розкладом! 😊🌾"
    )
...
async def broadcast(text, is_photo=False):
    for chat_id in load_groups():
        try:
            if is_photo:
                await bot.send_photo(chat_id=chat_id, photo=f"https://picsum.photos/800/600?random={random.randint(1, 10000)}", caption=text)
            else:
                await bot.send_message(chat_id=chat_id, text=text)
        except Exception as e:
            logging.error(f"Не вдалося відправити в {chat_id}: {e}")

async def job_morning():
    await broadcast(f"☀️ Доброго ранку! Сьогодні {get_day_info()}.\n{random.choice(WISDOMS)}", is_photo=True)

async def job_joke():
    await broadcast(f"😂 Анекдот дня: {random.choice(JOKES)}")

async def job_night():
    await broadcast("🌙 На добраніч! Солодких снів! 💤")
